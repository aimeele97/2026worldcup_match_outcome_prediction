...
The tournament was simulated using Monte Carlo methods to estimate each team's probability of winning the World Cup. The simulation used the Elo rating formula to calculate base win probabilities for the 8 quarterfinal teams, based on the XGBoost model's predictions. With 1,000 simulations, Spain had the highest estimated championship probability (21.60%), narrowly ahead of Argentina (21.10%). After increasing the simulation to 100,000 iterations, Argentina became the slight favourite (21.52%), with Spain close behind (21.40%). The difference between the two teams was only 0.12 percentage points, indicating that they are effectively evenly matched. The larger simulation produced more stable estimates, consistently identifying Argentina, Spain, France, and England as the tournament's strongest contenders.

### Model Convergence Analysis
Both the Monte Carlo simulation (1,000 trials) and XGBoost bracket predictions converge on the same top contenders:
